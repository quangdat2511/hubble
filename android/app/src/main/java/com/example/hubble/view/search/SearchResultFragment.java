...
public class SearchResultFragment extends Fragment {
    ...
    public enum Category {
        MESSAGES, MEMBERS, CHANNELS, FRIENDS
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        setupRecyclerView();
        setupSheetResultRefresh();
        observeViewModel();
    }

    private void setupSheetResultRefresh() {
        getChildFragmentManager().setFragmentResultListener(
                SearchUserPreviewBottomSheet.RESULT_KEY_RELATION_UPDATED,
                getViewLifecycleOwner(),
                (requestKey, result) -> {
                    if (!result.getBoolean(SearchUserPreviewBottomSheet.RESULT_BUNDLE_KEY_UPDATED, false)) {
                        return;
                    }
                    String query = viewModel.currentQuery.getValue();
                    viewModel.searchAll(query != null ? query : "");
                });
    }
